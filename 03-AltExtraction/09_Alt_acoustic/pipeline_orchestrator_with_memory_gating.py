...
class PipelineOrchestrator:
    ...
    def _wait_for_memory(self, required_gb: float = 15.0, check_interval: int = 300):
        ...
        if not PSUTIL_AVAILABLE:
            logging.warning("âš ï¸  psutil not available - skipping memory check")
        ...
        while True:
            ...
            if mem.available >= required_bytes:
                logging.info("="*80)
                logging.info(f"âœ“ MEMORY AVAILABLE: {available_gb:.1f}GB free (required: {required_gb}GB)")
            ...
            if first_check:
                logging.info("="*80)
                logging.info("â³ WAITING FOR MEMORY AVAILABILITY")
                logging.info("="*80)
            ...
            logging.info(f"[{timestamp}] â³ Waiting: {available_gb:.1f}GB free / {required_gb:.1f}GB needed")
    ...
    def run_pipeline(self):
        ...
        for stage_name, stage_func in stages:
            ...
            if self.state.is_stage_complete(stage_name):
                logging.info(f"\nâœ“ {stage_name} already complete, skipping")
                continue
    ...
    def _show_pipeline_status(self):
        ...
        for stage_name, stage_data in stages.items():
            ...
            if status == 'completed':
                logging.info(f"  âœ“ {stage_name}: COMPLETED")
            elif status == 'failed':
                logging.info(f"  âœ— {stage_name}: FAILED")
            elif status == 'in_progress':
                logging.info(f"  â‹¯ {stage_name}: IN PROGRESS")
        logging.info("")
    ...
    def generate_summary(self):
        ...
        if completed == total_stages:
            ...
            logging.info("âš ï¸  Pipeline completed with errors. Check logs for details.")
        else:
            logging.info("â„¹ï¸  Pipeline partially completed. Some stages may be pending.")
